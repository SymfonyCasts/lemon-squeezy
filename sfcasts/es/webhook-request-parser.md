...
A continuación, tenemos que calcular el hash de la carga útil de la petición utilizando el algoritmo de LemonSqueezy: `$payload = $request->getContent()`. Empezaremos con una variable hash: `$hash = hash_hmac('sha256', $payload, $secret)`. A continuación, tenemos que obtener la firma de la cabecera de la petición con `$signature = $request->headers->get('X-Signature', '')`.

Ahora es el momento de ver si el hash coincide con la firma. Si lo hace (`if (hash_equals($hash, $signature))`), entonces estamos listos. Si no, `throw new RejectWebhookException()` con un código de estado 401 `Response::HTTP_UNAUTHORIZED`. También añadiremos un mensaje - `Invalid LemonSqueezy signature!` - para saber qué ha pasado.

Una vez hecho esto, podemos llamar a esto desde el método `doParse()` del principio. Despejaré y eliminaré este código marcador de posición, y luego validaremos la carga útil: `$payload = $request->toArray()`. A continuación, diremos `$eventName = $payload['meta']['event_name']` y `$webhookId = $payload['meta']['webhook_id']`.

También añadiremos una comprobación de cordura para confirmar la presencia de `$eventName` y `webhookId` con `if (!$eventName || !$webhookId) {`. Por último, `throw new RejectWebhookException()`, le daremos un código de estado `Response::HTTP_BAD_REQUEST`, y añadiremos un mensaje de error: `Request payload does not contain required fields.`.

Bien, comprobemos si se trata de un evento admitido. De momento, sólo estamos siguiendo `order_created`. Si no lo es, lanzaremos otra excepción - `RejectWebhookException()` - con un estado `Response::HTTP_BAD_REQUEST`, y utilizaremos `sprintf('Unsupported event type: %s', $eventName)`.

Para terminar, devuelve un `new RemoteEvent($eventName, $webhookId, $payload)`. ¡Ya está! ¡Nuestro analizador sintáctico debería estar listo!

Si lo hemos hecho correctamente, LemonSqueezy debería recibir un código de estado 200 correcto. Podemos comprobarlo reenviando el webhook desde el panel de control de LemonSqueezy o reproduciendo el webhook desde la interfaz web de Ngrok. Haré clic en "Reproducir" y... ¡es un código de estado 202! ¡Éxito!
